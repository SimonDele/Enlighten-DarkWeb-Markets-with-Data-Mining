...
In the array you can find by columns our Prognoses and by rows the real sellers. In other words the diagonal shows the number of correct predictions and all other values are mistakes made by the algortihm.  
...
```
//
Results doesn't seem to be very good, the acuracy is `r ResultDT2[1]` % wich is lower than what we have previously with this kind of methods. It turns out that without the Seller, which give a lot information on the origin, prognoses are not very working and interesting.  

Let's see if correlation can be find between categories and origins.
//
