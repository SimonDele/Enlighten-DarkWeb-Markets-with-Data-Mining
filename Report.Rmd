...
We ran algorithm on the database with by rows ads and by columns the origin, category, seller and price. The aim is to predict who is selling each ads. We train the algorithm with half of the data and made predictions on other half. Given that most of the seller own just few ads (occasional advertisements) we have selected only the main ones, whose represent at best the market. Otherwise, data mining techniques will fail in finding rules for them.

**1. Decision tree**

First of all, we try to made prediction with decision tree method. In order to do this we use the *rpart package* which is based on the *CART Alorithm*. Here you can find our prognosis on the five most significant sellers and the related tree.
...
In the array you can find in columns our prediction and by rows the real seller. In other words the diagonal shows us the number of correct prediction and all others values are mistakes.  
The tree enable us to have a good visual aspect on predictions and gives us a lot of essentials informations on this five main sellers.

Besides, with quick basic statistics, it's possible to attest this tree and its branchs, that is to say how it's splited. Indeed let's have a look at this five deallers. This array shows us the main country where they are dealing, the main category that they are selling and the median price of their ads (i.e. 50% of ads are under this price and the other 50% are more expensive).
...
We can easly see that this informations are exactly the one that are in the tree. *jnenfrancis* is guessing with the origin *United Kingdom*. For the sellers dealing mainly in *China*, that is to say *ALaurizen* and *ROCKETLABS*, the spliting is made regarding the price (*ALaurizen* is obviously cheaper). And for others sellers the algorithm is based on the category *Cannabis & Hashish*.  

We have done here prediction on only five sellers in order to have a readable tree (otherwise the size of the tree is too big for being plot). However we can make a prognosis with more seller and still have good results. For instance with 10 seller we obtain the results below.
...
Once more, in this array you can find the number of corrects predictions and mistakes according to the different predicted seller.

**2. Bayesian classification - Naive alogrithm**

We used Bayesian Classification in the same objective as desicion tree,  making predictions. Here are results when running *Bayesian Naive Algorithm* with the same data
...
The accuracy is `r accBayesV1` % which is not very good comparing to decision tree. However one way to improve the prognosis is to add new variable to the data which could be relevant like the number of ads already sold and the creation date of the ad.
