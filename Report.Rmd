...
We ran algorithm on the database with by rows ads and by columns the origin, category, seller and price. The aim is to predict who is the seller of each ads. We train the algorithm with half of the data and made predictions on other half. Given that most of the seller own just few ads we have selected only the main ones. Otherwise, data mining techniques will fail in finding rules for them. Results can be found below.
...
CART Algorithm 
show the graph
sur % mais marche bien aussi sur plus ex :
...
We used Bayesian Classification in the same objective as desicion tree,  making predictions. We ran Bayesian Naive Algorithm with the same data and we obtain the results below.
...
The accuracy is `r accBayesV1` % which is not very good comparing to decision tree. To solve this problem we added new variable to the data which could be relevant like the number of ads already sold and the creation date of the ad.
...
```
Results show that the algorithm succeeds in predicting most of the sellers. Actually the accuracy is `r accBayesV2` %. Which is still a little bit less than with decision tree. However with more sellers (i.e more than 40 sellers), this algorithm tends to be more accurate than decision trees. We will see in the section Prediction of profitability, how to process with creation date of ads and number of sold products.

## - Clustering of drugs - Association Rules
