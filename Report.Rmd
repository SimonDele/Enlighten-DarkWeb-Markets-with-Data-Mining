...
As we said before, we will start by performing some basic statistics. Let's first analyse general distribution and trend of *AlphaBay Web Market*.
...
It is also worth noting that the second most popular category is *"Fraud"*, that is to say all the ads regarding impersonation, deception papers and accounts. It represents `r rate[2]` % of the market, which is smaller than the Drug market. Eventually, we can notice that all other items (digital product, weapons, jewelry ...) represent less than five percent of the marketplace.
...
**1. You can see here distribution of drugs.**
...
Circles show the amount of ads concerning drugs. The map confirms our assumptions that there are a lot of Drug dealers in Europe with, as major dealers countries, *United Kingdom*, *Netherlands* and *Germany*.
...
Globally, it appears that prices of *street sellers* are often largely higher than *AlphaBay* ads. In few cases both prices tend to be similar. (Please find in the last section all references that we used)
...
Please note that for all this part we only focus on *Drug Market*. 

## - Sellers Predictions 

The first thing we thought about was to guess the seller of an ad. We used therefore different data mining methods, especially *Decision Tree* and *Bayesian classification*.

We ran algorithm on the database with by rows ads and by columns the origin, category, seller and price. The aim is to predict who is selling each ads. We train the algorithm with half of the data and made predictions on other half. Given that most of sellers own just few ads (occasional advertisements) we have selected only the main ones, which represent at best the market. Otherwise, data mining techniques will fail in finding rules for them.

**1. Decision tree**

First of all, we try to make predictions with decision tree method. In order to do this we use the *rpart package* which is based on the *CART Alorithm*. Here you can find our prognosis on the five most significant sellers and the related tree.
...
It is striking to realise that our prediction is very reliable since the accuracy is `r accDT1` %.  
In the array you can find by columns our predictions and by rows the real sellers. In other words the diagonal shows us the number of correct predictions and all other values are mistakes made by the algortihm.  
The tree enables us to have a good visual aspect on predictions and gives us a lot of essential information on these five main sellers.
...
We can easily see that these information are exactly the ones that are in the tree. *jnenfrancis* is guessed thanks to origin *United Kingdom*. For the sellers dealing mainly in *China*, that is to say *ALaurizen* and *ROCKETLABS*, the spliting is made regarding the price (*ALaurizen* is obviously cheaper). For other sellers the algorithm use the category *Cannabis & Hashish*.  

We have done here predictions on only five sellers in order to have a readable tree (otherwise the size of the tree is too big for being plotted). However, we can make a prognosis with more sellers and still have good results. For instance, with 10 sellers, we obtain the results below.
...
Once more, in this array we can find the number of mistakes and correct predictions.

**2. Bayesian classification - Naive alogrithm**

We used Bayesian Classification in the same objective as decision tree,  making predictions. Here are results when running *Bayesian Naive Algorithm* with the same data.
...
```
Results show that the algorithm succeeds in predicting most of the sellers. Actually the accuracy is `r accBayesV2` %. Which is still a little bit less than with decision tree. However with more sellers (i.e more than 40 sellers for instance), this algorithm tends to be more accurate than the one based on decision tree method.  
We will see later in the section *Prediction of profitability*, how to exploit at best this two values : the *creation date of ads* and the *number of sold products*.

## - Clustering of drugs

Secondly, we wondered if there were links between some drugs. That is to say, if this is possible to cluster some drugs.  
...
Then, using *Association Rules* we have applied *Apriori algorithm*. We fixed one drug that must be in the itemset. Here it is Ecstasy and results can be seen below.
...
In this part we used a Bayesian Neural Network.
...
---
nocite: | 
  @Rmarkdown, @DrugWise, @tele, @Rehab, @cana, @stu
...
