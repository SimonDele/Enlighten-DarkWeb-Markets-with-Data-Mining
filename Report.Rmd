...
This also matches with the second chart that shows us drugs are the main item in the market. There are few exceptions such as *Afghanistan* which has been replaced by *Spain* and *Canada* has been reversed with *China*.  
...
Given that most of exported items are drugs, that is not surprising they are the most sold product as we saw it before. Once again this pie chart shows the market diversity. Although a huge part concerns *"Cannabis & Hashish"* category, we can notice that *"Psychedelics"* and other highly dangerous drugs are significantly present.  
...
Firstly, we calculated the average price of most common drugs and we obtained the result below.  
...
Secondly, we wanted to predict the profitability of an ad. That is to say, given an ad to predict if it will be sold a lot or not. Each ad have information on the category, origin, seller, price and a rate of profitability. This rate is caculated by dividing the number of product sold by the current lifetime of the ad and by times 30 to have a number of ads sold monthly. We ran Bayesian Neural Network algorithm on this ew data and we obtained the results below.
