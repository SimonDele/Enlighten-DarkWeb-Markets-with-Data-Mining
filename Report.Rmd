...
Our work consist in analysing this market, its nature, predominance of items, different origin (countires), main sellers ... Thus, over a first phase wee have performed some "Basic Statistics" on the *Data*, in order to discover the marketplace and to point out its trend. Then wee present some experimental results of *data mining* and discuss about it.
...
As wee said before wee started our research by performing some basics statics. Let's first analyse the general distibution, trend of the *AlaphaBay Web Market*.

**1. Here is a view of ads distribution in the world.**
...
```
This bar-chart represents the 10 main countries in the world regarding the number of ads. As we can see, United States is the biggest dealer far ahead of other. Their number of ads is more than twice as big as the number of the second (United Kingdom).  
Moreover, wee can notice that most of these countries belong to powerful area, that is to say countries from the north of America or from Europe, but we should not forget that China or Afghanistan are also present.  
...
To begin with, wee can note that there is 12 main categories in this web marketplace. At first sight, *"drugs and chemicals"* are the most important item dealt. In fact they represent `r rate[1]` % of the global market.  
It is also interesting to see that the second most popular item is *"Faud"*, that is to say all the ads regarding impersonation,deception papers and accounts. It represents `r rate[2]` % of the market (wich is very smaller compared with the Drug market). At last we can notice that all the other items (digital product, weapons, jewelry ...)  are under five thousands and represent a small part of the marketplace.

## - Drug Market

The *AlaphaBay Web Market* is well known to be a good place for dealing drugs, and wee can confirm it with our last chart. Therefore let's focus on the druf market.

**1. You can see here the diferent distribution of drugs.**
...
Wee can discern diferent type of drugs in the market wich is quite diversified.Nevertheless three main drugs : *Cannabis*, *Opioids* and *Ecstay* cover more than 50 % of the market. The other 50 % is plited by all the other drugs.

**2. Distribution of drugs in the world.**
...
Again wee are ploting the 10 main dealer-countries in the world but this time regarding the number of drugs ads. At first sight, the chart is very similar to the first one. And this is coherent, indeed by comparing the ratio between drugs ads and the total number of ads, we can deduce that they are mainly dealing drugs.  
This also match with the second chart that show us that drugs are the main item in the market. There are few exceptions such as *Afghanistan* which has been replaced by *Spain* and *Canad*a has been reversed with *China*.  
But what we can conclude so far is that the market of drugs is gathered in Europe and United States.
...
Let's now focus more specifically on diferents countries and study its tendency. To do this wee have had a look to the export and import flows of the country.

**1. United Kingdom exportation**

The pie below represents the repartition of each categories that United Kingdom exports. We focused on the second subcategory which appears to be the most relevant since the first one only gives information on the nature of the ad (for instance *"Drugs & Chemicals"*).
...
We can notice that most of exports items are drugs. That is not surprising since drugs are the products the most sold as wee saw it before. Once again this pie chart show the diversity of the market. Although, a huge part concerns *"Cannabis & Hashish"* we can notice that *"Psychedelics"* and other highly dangerous drugs are significantly present.  
We took the example of *United Kingdom* but most of European countries follow the same rules and this confirm what we have conclude in the section before.

**2. Let's have a look at the exportations of Afghanistan which are of a different kind than United Kingdom :**
...
What is suprising is that, unlike most of countries, *Afghanistan* don't really exports drugs. In fact most of the product that he is exporting are false identity, deception account... We can also notice that *Afghanistan* is also desling some electronic device or software.
...
It is noticeable that both are different. The percentage of each category is different and some of them doesn't appear systematicaly in the other pie chart. However it's obvious that significants drugs that are exports are also imports, and that *France* is importing some drugs that he doesn't have locally.   
Nevertheless we have to moderate our conclusions since target one particular country reduce significantly the number of informations we have for making statistics.

## - Market prices

After analysing the general trend and flows, on thing that is interesting to analyse is the market prices of course! We wondered if products sold in *AlphaBay* are cheapest than what is offer street sellers. 

**1. Average prices on the *AlphaBay Web Market* **

Firstly, we calculated the average price of the most common drugs and we obtain the result below.  
...
DrugsPrices <- function() {
  ...
  barp <- barplot(priceDrugs$med, main="Average Price of Drugs", names.arg = priceDrugs$drugs, xlim = c(0,max(priceDrugs$med+100)), cex.names = 0.8, col =rainbow(length(priceDrugs$drugs)), horiz =TRUE)
...
Finally, we were looking to some information  about the price of street sellers for same drugs as above. Thus, we gathered these information in articles and websites. Unfortunately, we failed to obtain information on some drugs. Below is the result of we found.
