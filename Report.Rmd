...
Globally, it appears that prices of *street sellers* are often largely higher than *AlphaBay* ads. In few cases both prices tend to be similar. (Please find in the last section all the sources that we used)
...
Please note that for all this part we only focus on the *Drug Market*. 
...
We ran algorithm on the database with by rows ads and by columns the origin, category, seller, and price. The aim is to predict who is the seller of each ads. We train the algorithm with half of the data and made predictions on other half. It's also important to stress the fact that we select only the main sellers  ? because, the remaining just own few ads ? in order to reduce the size of the data and boost our performance. Results can be found below.
...
We used Bayesian Classification in the same objective as desicion tree,  making predicting.

le principe est exatememt le meme blabla ... 
/!\ subset !!!! 
-> ici ne mettre que comme moi, mais, remettre un 2eme code en rajoutnt sol_since et products sold pour montrer que s'approche de DT et dire qu'on vera plus tard comment mieux utiliser ces 2 donnees 
/!\ affichage resultat !!!! 
...
acc2 <- BayesSellers()
sprintf("The accuracy is : %.2f %%", acc2)
```
Results show that the algorithm succeeds in predicting most of the sellers. Actually the accuracy is `r acc2` %. Which is a little bit less than with decision tree. However with more sellers (i.e more than 40 sellers), this algorithm tends to be more accurate than decision trees.
