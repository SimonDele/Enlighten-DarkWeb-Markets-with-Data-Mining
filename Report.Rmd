...
```{r echo=FALSE}
compCap <- figureCaption(compCap, "10 Main Dealer Countries")
```
...
```{r echo=FALSE}
compCap <- figureCaption(compCap, "AlphaBay Google Researchs in the World")
```
...
*AlphaBay Web Market* is a well known place for dealing drugs, this last chart has proved that. Moreover, its reputation can be reflected by looking at *Google* statistics @GTComp.  
![](.\Images\AlphaBayComparison.jpg)

```{r echo=FALSE}
compCap <- figureCaption(compCap, "Evolution of AlphaBay and Dream Market Google research")
```
...
```
```{r echo=FALSE}
compCap <- figureCaption(compCap, "10 Main Drug Dealer Countries")
```
...
```
```{r echo=FALSE}
compCap <- figureCaption(compCap, "European Drug Dealer")
```
...
```
```{r echo=FALSE}
compCap <- figureCaption(compCap, "Drugs Prices")
```
...
```
```{r echo=FALSE}
compCap <- figureCaption(compCap, "Comparison of Prices")
```
...
```{r echo=FALSE}
compCap <- figureCaption(compCap, "Seller Decision Tree")
```
...
```
```{r echo=FALSE}
compTab <- TableCaption(compTab, "Sellers Prediction / Bayesian Classification")
```
...
```
```{r echo=FALSE}
compCap <- figureCaption(compCap, "Drugs links")
```
...
```{r echo=FALSE}
compCap <- figureCaption(compCap, "Origin Decision Tree")
```
...
```
```{r echo=FALSE}
compCap <- figureCaption(compCap, "Rules origin")
```
...
```
```{r echo=FALSE}
compCap <- figureCaption(compCap, "Variable Dependencies")
```
